...
async def synthesize_voxcpm(request):
    ...
    try:
        ...
        async with _API_INFER_SEM:
            ...
            # Quality-retry fallback: if the reference contains multiple speakers
            # (or produced artifacts commonly caused by a mixed reference), try
            # to isolate the dominant speaker and regenerate once. This runs even
            # when enable_multi_speaker_check is False, so a poor dubbing
            # reference still has a chance to be salvaged during retry.
            if (
                quality_retry
                and severe
                and ref_audio_bytes is not None
                and ref_quality_profile is not None
                and not enable_multi_speaker_check
                and quality_retry_max >= 1
                and "empty" not in severe
                and (
                    "multi_speaker_reference" in (ref_quality_profile.get("issues") or [])
                    or bool(set(severe) & {"plosive", "harsh_high_freq", "impulsive_spike"})
                )
            ):
                logger.info(f"[{req_id}] quality retry: attempting dominant-speaker extraction")
                try:
                    extracted_bytes = await asyncio.to_thread(
                        _extract_dominant_speaker_reference, ref_audio_bytes
                    )
                ...
                if extracted_bytes:
                    extracted_quality = _assess_reference_quality(
                        extracted_bytes, enable_speaker_check=True
                    )
                    if "multi_speaker_reference" not in (extracted_quality.get("issues") or []):
                        ref_audio_bytes = extracted_bytes
                        ref_quality_profile = extracted_quality
                        ref_duration = _bytes_audio_duration(ref_audio_bytes)

                        if ref_temp_path is not None:
                            ref_temp_path.write_bytes(ref_audio_bytes)
                            ref_path = str(ref_temp_path)
                            gen_kwargs["ref_path"] = ref_path
                        ...
                        logger.info(
                            f"[{req_id}] quality retry dominant-speaker extraction succeeded: "
                            f"duration={extracted_quality.get('duration')}s, "
                            f"issues={extracted_quality.get('issues')}"
                        )
                        ...
                        try:
                            audio_waveform = await asyncio.to_thread(
                                _generate_voxcpm_sync, model, text, **gen_kwargs
                            )
                            _issues, spike_locations = _check_audio_quality_after_ceiling(
                                audio_waveform, sample_rate,
                                target_duration=target_duration_sec,
                            ...
                            logger.warning(
                                f"[{req_id}] regeneration after dominant-speaker extraction failed; "
                                f"keeping previous candidate: {exc}"
                            )
                    else:
                        logger.info(
                            f"[{req_id}] extracted segment still multi-speaker; "
